...
public class RedisSubscriber{
	 ...
	 * 
	 * @Override public void onMessage(Message message, byte[] pattern) { // 두번째
	 * 파라미터 : 채널 메칭을 위한 다양한 패턴 처리 try { // redis에서 발행된 데이터를 받아서 직렬화 String
	 * publishMessage = (String)
	 * redisTemplate.getStringSerializer().deserialize(message.getBody());
	 * 
	 * //ChatMessage객체로 매핑 ChatMessage roomMessage =
	 * objectMapper.readValue(publishMessage, ChatMessage.class);
	 * 
	...
	//redis에서 메시지가 발행되면 subscriber가 메시지 받아서 처리
